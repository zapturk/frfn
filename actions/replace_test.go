...
func TestReplaceFileName(t *testing.T) {
	...
	createTestDir := func(t *testing.T, files []string) string {
		...
		for _, file := range files { // 0666 is deprecated, use 0o666
	...
	t.Run("force replace", func(t *testing.T) {
		...
		expectedFiles := []string{"bar.txt", "test.txt"}
		...
		// Action
		err = actions.ReplaceFileName("foo", "bar", true)
		...
		expectedMsg := "foo.txt was changed to bar.txt"
	...
	t.Run("confirm with y", func(t *testing.T) {
		...
		// Action
		err = actions.ReplaceFileName("foo", "bar", false)
		...
		expectedMsg := "foo.txt will change to bar.txt"
		if !strings.Contains(output, expectedMsg) {
			t.Errorf("Expected output to contain %q, but got %q", expectedMsg, output)
		}
	})
	...
	t.Run("no matching files", func(t *testing.T) {
		...
		// Action
		err = actions.ReplaceFileName("nonexistent", "bar", true)
